...
export default function SalesOrdersPage() {
	...
	async function handleSubmit(e: React.FormEvent) {
		...
		if (!customerId || !createdByUserId) {
			notifications.show({
				...
				title: "Pedido",
				message:
					"Selecione cliente e quem registra o pedido.",
			});
		...
		if (lineItems.length === 0) {
			notifications.show({
				...
				title: "Pedido",
				message:
					"Inclua pelo menos um item (produto e quantidade).",
			});
	...
	const listError =
		ordersError && ordersErrorObj
			? apiErrorMessage(ordersErrorObj)
			: null;
